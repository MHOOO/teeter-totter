...
(ns
    ;; ^{:doc "Functions for binding the value of one thing to another, for example
    ;;        synchronizing an atom with changes to a slider."
    ;;   :author "Dave Ray, Thomas Karolski"}
...
(defn property 
  ...
  (reify Bindable
    (subscribe [this handler] 
      (let [fw (framework)]
        (-add-listener fw target property-kw handler) 
        (fn [] (-remove-listener fw target property-kw handler))))
    (notify [this v] (config! target property-kw v))))
...
(defn value 
  ...
  ([widget]
    (reify Bindable
      (subscribe [this handler]
        (throw (js/Error "Can't subscribe to value")))
      (notify [this v]
        (ssc/value! widget v)))))
...
(defn b-do*
  ...
  (reify Bindable
    (subscribe [this handler]
      (throw (js/Error. "Can't subscribe to b-do*")))
    (notify [this v]
      (apply f v args))))
...
(defn tee
  ...
  [& bindables]
  (reify Bindable
    (subscribe [this handler] (throw (js/Error. "tee bindables don't support subscription")))
